...
if (params.fastlmm == 1) {
  ...
  if(params.fastlmm_multi==1){
	...
	input:
     ...
     each file(file_rs) from filers_matrel_mat_fast
	output:
...
//genetic_map_file
  ...
  process doBoltmm{
    ...
    input:
      ...
      each file(rs_exclude) from rs_ch_exclude_bolt
      file(SnpChoiceMod) from filers_matrel
      each file(imp2_filelist) from Impute2FileList 
      each file(imp2_fid) from Impute2FID
      each file(bolt_ld_score) from Bolt_ld_score
      each file(bolt_genetic_map) from Bolt_genetic_map
...
if (params.gemma+params.gemma_gxe>0) {
  ...
  process getGemmaRel {
    ...
    input:
       ...
       each file(file_rs) from filers_matrel_mat_gem
    output:
 ...
 if(params.gemma_multi==1){
  ...
  process doGemmaChro{
    ...
    input:
      ...
      each file(rsfilelist) from rsfile
    each this_pheno from ind_pheno_cols_ch
...
if(params.gcta_grmfile==""){
 process FastGWADoGRM{
    ...
    input :
     ...
     each file(file_rs) from filers_matrel_mat_GWA
...
nextflowversion = "Nextflow v${nextflow.version}"
